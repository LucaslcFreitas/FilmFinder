...
export class ActorService {
    ...
    getPopularActors(page: number = 1): Observable<Response<Actor>> {
        const url = `${this.baseApiUrl}popular?api_key=${this.apiKey}&language=${this.language}&page=${page}`;

        return this.http.get<Response<Actor>>(url);
    }

    getActor(id: string): Observable<ActorDetails> {
        const url = `${this.baseApiUrl}${id}?api_key=${this.apiKey}&language=${this.language}&append_to_response=movie_credits`;
